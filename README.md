...
**当前为 PoC，不是成熟的自动执法或高空抛物溯源系统。** 远距离快速下落的小烟头尚不能可靠识别，不能自动确定来源楼层、窗口或责任人。未检出不代表没有发生事件。仓库仅包含下方经提供者授权公开的两张连续帧示例图，不包含原始现场录像、其他抓拍、住户资料、运行日志、数据库或模型权重。

## 真实录像回查示例

以下两张图片由项目使用者提供并授权公开，来自真实录像回查，按上传原图展示，未作生成式修改。两图均为 **4 列 × 3 行、12 帧**的连续截图（不是 3×3 九宫格），按从左到右、从上到下的顺序阅读。

可见细长小物体在连续画面中下落与翻转，**疑似烟头；本案例来源楼层已由拍摄者人工确认为 11 楼**。楼层结论由拍摄者提供，不是系统自动识别结果，也不是仅凭下列截图推断。示例用于展示逐帧回查效果，不代表模型已经自动识别或准确溯源。

### 示例一：约 11:39:33.700–11:39:34.067

![疑似烟头下落连续帧示例一，12帧，来源楼层11楼由拍摄者人工确认](docs/examples/sequence-113933.png)

### 示例二：约 11:52:26.300–11:52:26.667

![疑似烟头下落连续帧示例二，12帧，来源楼层11楼由拍摄者人工确认](docs/examples/sequence-115226.png)

图上的时间来自录像开始时间与帧时间推算，用于帧间对照；显示到毫秒不代表实际钟表时间精确到毫秒。原始录像与慢放视频未随这两张图片一并公开。新版回查命令支持另行导出 3×3 九宫格及 6 倍慢放视频，见 [回查指南](docs/investigation-workflow.md)。
...
支持人工按楼层范围组织拍摄，例如先观察 1–10 楼，再观察 10–20 楼；也可用两台电脑、两部摄像机同时覆盖并保留重叠区。**分时段未检出不能直接排除楼层**，双机需要记录时钟偏差，当前不具备自动双机轨迹融合或楼层识别。

发现地面烟头后，可触发回查此前数小时的已关闭录像，筛选运动候选，生成 **3×3 九宫格图片**和 **6 倍慢放视频（1/6 速度）**：

```bash
python -m app.review recent '/path/to/recordings/camera01' outputs/recent-review \
  --discovered-at '2026-09-19T13:00:00' --hours 3 --max-candidates 12
```

发现时间只是回查截止点，不是掉落时间。程序报告时间缺口和失败，输出仍需人工确认；尚未与实时检测自动联动。完整双机操作、文件命名、输出说明与单片段导出方法见 [分区排查与回查指南](docs/investigation-workflow.md)。
...
本项目源码以 **GNU AGPL-3.0-only** 发布，见 [LICENSE](LICENSE)。依赖与模型保留其各自许可证，参见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。不随仓库分发模型权重或原始现场录像；两张授权公开示例图见上文；模型来源链接及初始化方法见第三方说明与上文。
